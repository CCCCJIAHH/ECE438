//
// Created by jiahan Chang on 2021-10-18.
//
#include <stdio.h>
#include <stdlib.h>
#include <errno.h>
...
unsigned long long int readLen;
...
void fillBuffer(int pkt_number);
...
void reliablyTransfer(char* hostname, unsigned short int hostUDPport, char* filename, unsigned long long int bytesToTransfer) {
    ...
    while (!buffer.empty() || !wait_ack.empty()) {
        if((numbytes = recvfrom(socket, pkt_buffer, sizeof(packet), 0, NULL, NULL)) == -1) {
            ...
            cout << "Time out, resend packet " << wait_ack.front().seq_num << endl;
            ...
            if((numbytes = sendto(socket, pkt_buffer, sizeof(packet), 0, p->ai_addr, p->ai_addrlen))== -1){
                ...
                printf("Fail to send %d packet", wait_ack.front().seq_num);
                exit(2);
            }
            ...
            if (pkt.msg_type == ACK) {
                ...
                if (pkt.ack_num == wait_ack.front().seq_num) {
                    ...
                    if (dupAckCount == 3) {
                        ...
                        cout << "3 duplicate ACKs, resend packet " << wait_ack.front().seq_num << endl;
                    }
...
void sendPkts(int socket) {
    ...
    if (cwnd - wait_ack.size() < 1) {
        ...
        cout << "Sent packet "<< wait_ack.front().seq_num << " cwnd = "<< cwnd << endl;
        return;
    }
    if (buffer.empty()) {
    ...
    for (int i = 0; i < pkts_to_send; ++i) {
        ...
        if((numbytes = sendto(socket, pkt_buffer, sizeof(packet), 0, p->ai_addr, p->ai_addrlen))== -1){
            perror("Error: data sending");
            printf("Fail to send %d packet", buffer.front().seq_num);
            exit(2);
        }
        cout << "Sent packet "<< buffer.front().seq_num << " cwnd = "<< cwnd << endl;
...
//    int count = 0;
    for (int i = 0; readLen!= 0 && i < pkt_number; ++i) {
        packet pkt;
...
//the fucntion of congestionControl
void congestionControl(bool newACK, bool timeout) {
